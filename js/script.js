document.addEventListener('DOMContentLoaded', () => {
    ...
    const shortPriceToggle = document.getElementById('short-price-toggle');
    ...
    bulkAddBtn.addEventListener('click', () => {
        ...
        lines.forEach(line => {
            ...
            if (parts.length >= 2) {
                ...
                // Nếu giá hợp lệ, kiểm giá rút gọn
                if (!isNaN(price)) {
                    if (shortPriceToggle.checked && price < 1000) {
                        price = price * 1000;
                    }
                } else {
                    return; // Nếu giá không hợp lệ, bỏ qua dòng này
                }
    ...
    generateBtn.addEventListener('click', () => {
        ...
        try {
            ...
            const carInfo = {
                ...
                plate: carPlateInput.value.trim()
            };

            // Generate receipt using Canvas (client-side)
            const canvas = generateReceiptCanvas(items, total, paid, change, carInfo);
