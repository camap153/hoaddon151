...
function generateReceiptCanvas(items, total, paid, change, carInfo = null) {
    ...
    if (carInfo && (carInfo.name || carInfo.plate)) {
        ctx.font = 'bold 13px Arial, sans-serif';
        if (carInfo.name) {
            ctx.fillText(`Xe: ${carInfo.name}`, 10, y);
            y += 18;
        }
        if (carInfo.plate) {
            ctx.fillText(`BS: ${carInfo.plate}`, 10, y);
            y += 18;
        }
        y += 4;
    }
